...
public class BoardSnapshotFormatter {
    ...
    public String formatWithSearch(GameBoard board, Piece.Side side, List<SearchAnalysis> analyses, boolean inCheck) {
        StringBuilder builder = new StringBuilder(format(board, side,
                analyses.stream().map(SearchAnalysis::candidate).toList(), inCheck));
        builder.append("search_ranking:\n");
        for (SearchAnalysis analysis : analyses) {
            CandidateMove candidate = analysis.candidate();
            builder.append(candidate.id())
                    .append(' ')
                    .append(candidate.kind())
                    .append(' ')
